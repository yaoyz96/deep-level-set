...
def extreme_points(mask, pert):
    ids_y, ids_x = np.where(mask > 0.5)

    def find_points(idx, idy, ids):
        id = ids[0][random.randint(0, len(ids[0]) - 1)]
        return [idx[id], idy[id]]

    # radia
    ran_ = np.random.rand(8, 1)
    ran_[np.where(ran_ >= 0.5)[0]] = 1
    ran_[np.where(ran_ < 0.5)[0]] = -1

    # distance
    dxy = np.ones((4, 1))
    for i in range(4):
        dxy[i] = pert * np.random.rand() * ran_[i]

    lx = find_points(ids_x, ids_y, np.where(ids_x <= np.min(ids_x)))  # left
    rx = find_points(ids_x, ids_y, np.where(ids_x >= np.max(ids_x)))  # right
    ty = find_points(ids_x, ids_y, np.where(ids_y <= np.min(ids_y)))  # top
    by = find_points(ids_x, ids_y, np.where(ids_y >= np.max(ids_y)))  # bottom

    lx[0] += dxy[0]
    lx[1] += np.sqrt(pert ** 2 - dxy[0] * 2) * np.random.rand() * ran_[4]
    rx[0] += dxy[1]
    rx[1] += np.sqrt(pert ** 2 - dxy[1] * 2) * np.random.rand() * ran_[5]
    ty[0] += dxy[2]
    ty[1] += np.sqrt(pert ** 2 - dxy[2] * 2) * np.random.rand() * ran_[6]
    by[0] += dxy[3]
    by[1] += np.sqrt(pert ** 2 - dxy[3] * 2) * np.random.rand() * ran_[7]
    ext_points = np.array([[int(lx[0]), int(lx[1])], [int(rx[0]), int(rx[1])],
                           [int(ty[0]), int(ty[1])], [int(by[0]), int(by[1])]])
    return ext_points
...
def crop_from_bbox(img, bbox, zero_pad=False):
    ...
    if len(bbox) > 4:
        ...
        bbox[2] = float(data[3])
        bbox[3] = float(data[4])
        bbox[0] = int(float(bbox[0]) * img.shape[1])
        bbox[1] = int(float(bbox[1]) * img.shape[0])
        bbox[2] = int(float(bbox[2]) * img.shape[1]) + bbox[0]
        bbox[3] = int(float(bbox[3]) * img.shape[0]) + bbox[1]
    ...
    if img.ndim == 2:
        ...
        crop = np.tile(crop[:, :, np.newaxis], [1, 1, img.shape[2]])
        crop[inds[1]:inds[3] + 1, inds[0]:inds[2] + 1] = \
            img[bbox_valid[1]:bbox_valid[3] + 1, bbox_valid[0]:bbox_valid[2] + 1]
    else:
