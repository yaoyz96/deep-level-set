...
class AutoRefuge(Dataset):

    def __init__(self, train=True, split='train',
                 db_root_dir=Path.db_root_dir('refuge'),
                 transform=None,
                 retname=True):
    ...
    def __getitem__(self, idx):
        ...
        bbox_name = os.path.split(ann['im_path'])[-1]
        name = bbox_name.split('.')[0] + '.txt'
        bbox_path = self.db_root_dir + '/box/' + name
        data = []
        f = open(bbox_path, 'r')
        data = f.readlines()
        f.close()

        sample = {'image': img, 'gt': gt, 'bbox': data}
